...
@views function assign!(dest, src)
    for loc = eachindex(dest)
        for dir = eachindex(dest[loc])
...
@views function apply_preconditioner(Z, R, A, η, dx, dy, γ)
    ...
    @. Z.vc.x = R.vc.x / (
        2 * (η.c[2:end-2, 2:end-1] + η.c[3:end-1, 2:end-1]) / dx^2
          + (η.v[2:end-1, 1:end-1] + η.v[2:end-1, 2:end]) / dy^2
          + 2γ / dx^2)

    @. Z.vc.y = R.vc.y / (
        2 * (η.v[:, 1:end-1] + η.v[:, 2:end]) / dy^2
          + (η.c[1:end-1, 2:end-1] + η.c[2:end, 2:end-1]) / dx^2
          + 2γ / dy^2)

    @. Z.cv.x = R.cv.x / (
        2 * (η.v[1:end-1, :] + η.v[2:end, :]) / dx^2
          + (η.c[2:end-1, 1:end-1] + η.c[2:end-1, 2:end]) / dy^2
          + 2γ / dx^2)

    @. Z.cv.y = R.cv.y / (
        2 * (η.c[2:end-1, 2:end-2] + η.c[2:end-1, 3:end-1]) / dy^2
...
@views function main()
    ...
    for iter in 1:maxiter
        R̄ = Enzyme.make_zero(R)
        V̄ = Enzyme.make_zero(V)
        P̄ = Enzyme.make_zero(P)
        τ̄ = Enzyme.make_zero(τ)
        ...
        autodiff(Forward, residual!, DuplicatedNoNeed(R, R̄), Duplicated(V, V̄), Duplicated(P, P̄), Const(P_old),
                 Duplicated(τ, τ̄), Const(A), Const(η), Const(ρg), Const(γ), Const(dx), Const(dy))
        α = δ / dot_product(D, R̄)
