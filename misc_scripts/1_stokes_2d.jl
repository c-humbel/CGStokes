...
@views function J(V, P, P_old, ∇V, τ, ε̇, A, η, ρg, ω, γ, dx, dy)
    ...
    return 0.5 * (sum(τ.c.xx .* ε̇.c.xx .* ω.c[2:end-1, 2:end-1]) + sum(τ.c.yy .* ε̇.c.yy .* ω.c[2:end-1, 2:end-1])) + sum(τ.c.xy .* ε̇.c.xy .* ω.c) +
           0.5 * (sum(τ.v.xx .* ε̇.v.xx .* ω.v) + sum(τ.v.yy .* ε̇.v.yy .* ω.v)) + sum(τ.v.xy .* ε̇.v.xy .* ω.v) -
           sum((P_old.c .- 0.5γ .* ∇V.c) .* ∇V.c .* ω.c[2:end-1, 2:end-1]) - sum((P_old.v .- 0.5γ .* ∇V.v) .* ∇V.v .* ω.v) +
           sum(ρg.vc.x .* V.vc.x[2:end-1, :] .* ω.vc.x) + sum(ρg.vc.y .* V.vc.y[:, 2:end-1] .* ω.vc.y) +
           sum(ρg.cv.x .* V.cv.x[2:end-1, :] .* ω.cv.x) + sum(ρg.cv.y .* V.cv.y[:, 2:end-1] .* ω.cv.y)
end

@views function residual!(R, V, P, P_old, ∇V, τ, ε̇, A, η, ρg, ω, γ, dx, dy)
    ...
    @. R.vc.x = (P.c[2:end, :] * ω.c[3:end-1, 2:end-1] - P.c[1:end-1, :] * ω.c[2:end-2, 2:end-1]) / dx -
                (τ.c.xx[2:end, :] * ω.c[3:end-1, 2:end-1] - τ.c.xx[1:end-1, :] * ω.c[2:end-2, 2:end-1]) / dx -
                (τ.v.xy[2:end-1, 2:end] * ω.v[2:end-1, 2:end] - τ.v.xy[2:end-1, 1:end-1] * ω.v[2:end-1, 1:end-1]) / dy +
                ρg.vc.x * ω.vc.x
    @. R.cv.y = (P.c[:, 2:end] * ω.c[2:end-1, 3:end-1] - P.c[:, 1:end-1] * ω.c[2:end-1, 2:end-2]) / dy -
                (τ.c.yy[:, 2:end] * ω.c[2:end-1, 3:end-1] - τ.c.yy[:, 1:end-1] * ω.c[2:end-1, 2:end-2]) / dy -
                (τ.v.xy[2:end, 2:end-1] * ω.v[2:end, 2:end-1] - τ.v.xy[1:end-1, 2:end-1] * ω.v[1:end-1, 2:end-1]) / dx +
                ρg.cv.y * ω.cv.y

    @. R.cv.x = (P.v[2:end, :] * ω.v[2:end, :] - P.v[1:end-1, :] * ω.v[1:end-1, :]) / dx -
                (τ.v.xx[2:end, :] * ω.v[2:end, :] - τ.v.xx[1:end-1, :] * ω.v[1:end-1, :]) / dx -
                (τ.c.xy[2:end-1, 2:end] * ω.c[2:end-1, 2:end] - τ.c.xy[2:end-1, 1:end-1] * ω.c[2:end-1, 1:end-1]) / dy +
                ρg.cv.x * ω.cv.x

    @. R.vc.y = (P.v[:, 2:end] * ω.v[:, 2:end] - P.v[:, 1:end-1] * ω.v[:, 1:end-1]) / dy -
                (τ.v.yy[:, 2:end] * ω.v[:, 2:end] - τ.v.yy[:, 1:end-1] * ω.v[:, 1:end-1]) / dy -
                (τ.c.xy[2:end, 2:end-1] * ω.c[2:end, 2:end-1] - τ.c.xy[1:end-1, 2:end-1] * ω.c[1:end-1, 2:end-1]) / dx +
                ρg.vc.y * ω.vc.y
...
@views function line_search(R, R̄, V, V̄, P, P̄, P_old, ∇V, ∇V̄, τ, τ̄, ε̇, ε̄, D, A, η, ρg, ω, γ, δ, dx, dy)
    make_zero!(R̄)
    make_zero!(V̄)
    make_zero!(P̄)
    make_zero!(τ̄)
    ...
    autodiff(set_runtime_activity(Forward),
             ...
             Duplicated(τ, τ̄),
             Duplicated(ε̇, ε̄),
             Const(A), Const(η), Const(ρg), Const(ω), Const(γ), Const(dx), Const(dy))
...
@views function main()
    ...
    # arrays
    # volume fractions
    ω = (c  = zeros(nx + 2, ny + 2), v  = zeros(nx + 1, ny + 1),
    ...
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.c[2:end-1, 2:end-1], xc, yc')
    @. ω.c[[1, end], :] = ω.c[[2, end - 1], :]
    @. ω.c[:, [1, end]] = ω.c[:, [2, end - 1]]
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.v, xv, yv')
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.vc.x, xv[2:end-1], yc')
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.cv.y, xc, yv[2:end-1]')
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.cv.x, xc, yv')
    broadcast!((x, y) -> incf(x, y, xf, yf, rf, 0.0, 1.0), ω.vc.y, xv, yc')
    # plots
    ...
    hms = (heatmap!(axs[1], xc, yc, A.c; colormap=:roma),
           ...
           heatmap!(axs[4], xc, yc, ω.c; colormap=Makie.Reverse(:grays)))
    ...
    for iter_ph in 1:maxiter_ph
        ...
        # velocity solver
        # init residual
        residual!(R, V, P, P_old, ∇V, τ, ε̇, A, η, ρg, ω, γ, dx, dy)
        apply_preconditioner(Z, R, A, η, ω, dx, dy, γ)
        # init search direction
        assign!(D, Z)
        δ = dot_product(R, Z)
        # CG iterative loop
        for iter in 1:maxiter
            α = line_search(R, R̄, V, V̄, P, P̄, P_old, ∇V, ∇V̄, τ, τ̄, ε̇, ε̄, D, A, η, ρg, ω, γ, δ, dx, dy)
        ...
        err_Pr = (maximum(abs.(∇V.c .* ω.c[2:end-1, 2:end-1])),
                  maximum(abs.(∇V.v .* ω.v)))
        ...
        hms[2][3] = P.c .* ω.c[2:end-1, 2:end-1]
        hms[3][3] = V.vc.y[:, 2:end-1] .* ω.vc.y
        display(fig)
    end
